...
public abstract class FFmpegDownloader
{
    ...
    /// </summary>
    public async static Task GetLatestVersion(FFmpegVersion version, IProgress<ProgressInfo> progress = null)
    {
        await GetLatestVersion(version, null, progress);
    }

    /// <summary>
    ///     Download latest FFmpeg version for current operating system to "." directory. Works best with
    ///     FFmpeg.ExecutablesPath as path
    ///     <param name="version">Determine which version of FFmpeg should be downloaded</param>
    ///     <param name="path">FFmpeg executables destination directory</param>
    ///     <param name="progress">Progress of download</param>
    /// </summary>
    public async static Task GetLatestVersion(FFmpegVersion version, string path, IProgress<ProgressInfo> progress = null)
    {
