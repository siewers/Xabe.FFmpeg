...
{
    internal SharedFFmpegDownloader()
    {
    }

    internal SharedFFmpegDownloader(IOperatingSystemProvider operatingSystemProvider)
    ...
    protected override void Extract(string ffMpegZipPath, string destinationDir)
    {
        using (var zipArchive = ZipFile.OpenRead(ffMpegZipPath))
        {
            ...
            foreach (var zipEntry in zipArchive.Entries.Where(item => item.FullName.Contains("bin")))
            {
