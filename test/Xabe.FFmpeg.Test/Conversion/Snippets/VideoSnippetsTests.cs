    ...
    : IClassFixture<StorageFixture>, IClassFixture<MediaMtxServerFixture>
{
    ...
    {
        var output = storageFixture.CreateMediaLocation().WithExtension(FileExtensions.Mkv);
    ...
    {
        var output = storageFixture.CreateMediaLocation().WithExtension(FileExtensions.Mp4);
        await FFmpeg.Conversions.FromSnippet.ExtractVideo(Resources.Mp4WithAudio, output, _testCancellationToken)
                    .StartConversion(_testCancellationToken);
    ...
    {
        var output = storageFixture.CreateMediaLocation().WithExtension(FileExtensions.Png);
        await Assert.ThrowsAsync<ArgumentException>(() => FFmpeg.Conversions.FromSnippet
                                                                .Snapshot(Resources.Mp4WithAudio, output, 999.Seconds(), _testCancellationToken)
                                                                .StartConversion(_testCancellationToken)
                                                   );
    }
    ...
    {
        var output = storageFixture.CreateMediaLocation().WithExtension(extension);
        await FFmpeg.Conversions.FromSnippet.Snapshot(Resources.Mp4WithAudio, output, 0.Seconds(), _testCancellationToken)
                    .StartConversion(_testCancellationToken);
    ...
    public async Task WatermarkTest()
    {
        ...
        var result = await FFmpeg.Conversions.FromSnippet
                                 .SetWatermark(Resources.Mp4WithAudio, output, Resources.PngSample, Position.Center, _testCancellationToken)
                                 .StartConversion(_testCancellationToken);
    ...
    public async Task BasicConversion_InputFileWithSubtitles_SkipSubtitles()
    {
        var output = storageFixture.CreateMediaLocation().WithExtension(FileExtensions.Mp4);
        await FFmpeg.Conversions.FromSnippet
                    .Convert(Resources.MkvWithSubtitles, output, cancellationToken: _testCancellationToken)
                    .StartConversion(_testCancellationToken);
    ...
    public async Task BasicConversion_InputFileWithSubtitles_SkipSubtitlesWithParameter()
    {
        var output = storageFixture.CreateMediaLocation().WithExtension(FileExtensions.Mp4);
        await FFmpeg.Conversions.FromSnippet
                    .Convert(Resources.MkvWithSubtitles, output, keepSubtitles: false, _testCancellationToken)
                    .StartConversion(_testCancellationToken);
    ...
    public async Task BasicConversion_InputFileWithSubtitles_KeepSubtitles()
    {
        var output = storageFixture.CreateMediaLocation().WithExtension(FileExtensions.Mp4);
        await FFmpeg.Conversions.FromSnippet
                    .Convert(Resources.MkvWithSubtitles, output, keepSubtitles: true, _testCancellationToken)
                    .StartConversion(_testCancellationToken);
    ...
    public async Task BasicTranscode_InputFileWithSubtitles_KeepSubtitles(VideoCodec videoCodec, AudioCodec audioCodec, SubtitleCodec subtitleCodec)
    {
        var output = storageFixture.CreateMediaLocation().WithExtension(FileExtensions.Mp4);
        await FFmpeg.Conversions.FromSnippet
                    .Transcode(Resources.MkvWithSubtitles, output, videoCodec, audioCodec, subtitleCodec, keepSubtitles: true, _testCancellationToken)
                    .StartConversion(_testCancellationToken);
    ...
    public async Task BasicTranscode_InputFileWithSubtitles_SkipSubtitles(VideoCodec videoCodec, AudioCodec audioCodec, SubtitleCodec subtitleCodec)
    {
        var output = storageFixture.CreateMediaLocation().WithExtension(FileExtensions.Mp4);
        await FFmpeg.Conversions.FromSnippet
                    .Transcode(Resources.MkvWithSubtitles, output, videoCodec, audioCodec, subtitleCodec, keepSubtitles: false, _testCancellationToken)
                    .StartConversion(_testCancellationToken);
    ...
    public async Task BasicConversion_SloMoVideo_CorrectFramerate()
    {
        var output = storageFixture.CreateMediaLocation().WithExtension(FileExtensions.Mp4);
        await FFmpeg.Conversions.FromSnippet
                    .Convert(Resources.SloMoMp4, output, keepSubtitles: false, _testCancellationToken)
                    .StartConversion(_testCancellationToken);
    ...
    public async Task BasicConversion_InputFileWithMultipleStreams_CorrectResult()
    {
        var output = storageFixture.CreateMediaLocation().WithExtension(FileExtensions.Mp4);
        await FFmpeg.Conversions.FromSnippet
                    .Convert(Resources.MultipleStream, output, keepSubtitles: false, _testCancellationToken)
                    .StartConversion(_testCancellationToken);
